...
public class Turtle {
  ...
  public Turtle(GraphicsContext gc, double xHome, double yHome, double pauseInSeconds) {
    ...
    // Fake-Aktivität, damit Animation auch mit nur einer Aktivität startet
    this.forward(0);
  }
  ...
  public void setRandomColor() {
    Color[] colors = { Color.BLUE, Color.BLACK, Color.CORAL, Color.CYAN, Color.DARKGREY, Color.DEEPSKYBLUE };
...
// Draw Circle (Custom)
  public void drawCircle(double radius) {
    if (penDownFlag) {
        ...
        timeline.getKeyFrames().add(new KeyFrame(timepoint, e -> gc.strokeOval(xTurtle - radius, yTurtle - radius, diameter, diameter)));
        timepoint = timepoint.add(pause);
    }
  }
...
// Draw Sign Wave (Custom)
	public void drawWave(double length, double amplitude, int numPoints, String waveType) {
		if (penDownFlag) {
			double step = length / numPoints;

			// Initialize the path
			timeline.getKeyFrames().add(new KeyFrame(timepoint, e -> {
				gc.beginPath();
				gc.moveTo(xTurtle, yTurtle);
			}));

			for (int i = 0; i <= numPoints; i++) {
				double x = xTurtle + i * step;
				double y;

				switch (waveType.toLowerCase()) {
					case "sin":
						y = yTurtle - amplitude * Math.sin((2 * Math.PI * i) / numPoints);
						break;
					case "cos":
						y = yTurtle - amplitude * Math.cos((2 * Math.PI * i) / numPoints);
						break;
					// Add more cases for other wave types if needed
					default:
				...
				// Update the turtle's position within the keyframe
				timeline.getKeyFrames().add(new KeyFrame(timepoint, e -> gc.lineTo(x, y)));
			}

			// Draw the path at the end of the animation
			timeline.getKeyFrames().add(new KeyFrame(timepoint, e -> gc.stroke()));
...
// Rectangle (Custom)
	public void drawFilledRectangle(double x, double y, double width, double height) {
		if (penDownFlag) {
			timeline.getKeyFrames().add(new KeyFrame(timepoint, e -> gc.fillRect(x, y, width, height)));
			timepoint = timepoint.add(pause);
		}
	}
